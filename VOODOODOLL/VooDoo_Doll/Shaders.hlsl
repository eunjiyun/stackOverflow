...
VS_STANDARD_OUTPUT VSStandard(VS_STANDARD_INPUT input)
{
	...
	//output.color= mul(input.normal, (float3x3)gmtxGameObject);
...
float4 PSStandard(VS_STANDARD_OUTPUT input) : SV_TARGET
{
	...
	{
		//float3x3 TBN = float3x3(normalize(input.tangentW), normalize(input.bitangentW), normalize(input.color));
...
//VS_STANDARD_OUTPUT VSLighting(VS_SKINNED_STANDARD_INPUT input)
{
	//VS_STANDARD_OUTPUT output;
	...
	output.normalW = mul(input.normal, (float3x3)gmtxGameObject);
	//output.normalW = mul(input.color, (float3x3)gmtxGameObject);
...
VS_SHADOW_MAP_OUTPUT VSShadowMapShadow(VS_LIGHTING_INPUT input)//0401
//VS_SHADOW_MAP_OUTPUT VSShadowMapShadow(VS_SKINNED_STANDARD_INPUT input)//0401
{
	...
	//output.normalW = mul(float4(input.color, 0.0f), gmtxGameObject).xyz;
	output.normalW = mul(float4(input.normal, 0.0f), gmtxGameObject).xyz;

	

	/*if(round(input.normal.x)==0 && round(input.normal.y)== 0 && round(input.normal.z) == 0)
		output.normalW = mul(float4(0.0f, 1.0f, 0.0f, 0.0f), gmtxGameObject).xyz;
	else*/
	...
	//	mtxVertexToBoneWorld += input.weights[i] * mul(gpmtxBoneOffsets[input.indices[i]], gpmtxBoneTransforms[input.indices[i]]);
	//}
	////output.normalW = mul(input.normal, (float3x3)mtxVertexToBoneWorld).xyz;//0401
	////output.normalW = input.normal / (float3x3)mtxVertexToBoneWorld).xyz;

	//// input.normal을 계산하는 코드
	//float4x4 mtxVertexToBoneWorldInvTranspose = transpose(inverse((float4x4)mtxVertexToBoneWorld));
	//float3 normal = mul(output.normalW, mtxVertexToBoneWorldInvTranspose);



		//output.normalW = mul(float4(normal, 0.0f), gmtxGameObject).xyz;
		...
		//	//		mtxVertexToBoneWorld += input.weights[i] * gpmtxBoneTransforms[input.indices[i]];
		//	mtxVertexToBoneWorld += input.weights[i] * mul(gpmtxBoneOffsets[input.indices[i]], gpmtxBoneTransforms[input.indices[i]]);
		//	//mtxVertexToBoneWorld += 50 * mul(gpmtxBoneOffsets[input.indices[i]], gpmtxBoneTransforms[input.indices[i]]);
		//}
