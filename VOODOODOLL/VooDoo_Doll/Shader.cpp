...
D3D12_INPUT_LAYOUT_DESC CStandardShader::CreateInputLayout()
{
	...
	pd3dInputElementDescs[1] = { "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 1, 0, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };//0313 이거?
	pd3dInputElementDescs[2] = { "NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT, 2, 0, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };//0313
...
void CDepthRenderShader::PrepareShadowMap(ID3D12GraphicsCommandList* pd3dCommandList,LIGHT* m_pLights)
{
	for (int j = 0; j < MAX_SHADOW_LIGHTS; j++)
	{
		if (m_pLights[j].m_bEnable)
		{
			...
			pd3dCommandList->OMSetRenderTargets(5, &m_pd3dRtvCPUDescriptorHandles[j], TRUE, &m_d3dDsvDescriptorCPUHandle);
